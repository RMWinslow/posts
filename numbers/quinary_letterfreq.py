...
#%% find the best quinary sets
cleaned_subset_counts = Counter()
for combo in combinations("bcdfghjklmnpqrstvwxyz",5):
    letters = list('aeiou') + list(combo)
    cleaned_subset_counts["".join(list(combo))] = cleaned_wordScore(letters)
print(cleaned_subset_counts.most_common()[:10])
...
Results for Quinary case, no restrictions on consonants. y and w allowed:
Brown:
Alternation: fnrst
Alteration which begins/ends with vowel: fnrst
Alteration which ends with vowel: bhmrt
...
Using all the consonants in brown corpus, without alternation restriction:
