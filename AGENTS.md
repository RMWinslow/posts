...
- `_config.yml` - Jekyll config, remote theme, plugins (jekyll-redirect-from, jekyll-sitemap),
  and `exclude` list (currently excludes `AGENTS.md` from site rendering)
...
### Options for working across repos with Codex

**Option A: Run Codex from each repo independently (current approach)**
- Simple. Each session has its own AGENTS.md with repo-specific context.
...
- Run Codex from `C:\Users\rober\Documents\GitHub\` with access to all repos.
- Could use a AGENTS.md there that links to per-repo docs.
- Downside: Not a git repo, so Codex's git-aware features (status, diffing)
...
- Create e.g. `C:\Users\rober\Documents\GitHub\website\` containing just the
  ...
  of JTD-RMW). Put a AGENTS.md there.
...
- Stay in per-repo sessions (Option A), but `git clone` JTD-RMW locally so
  Codex can read/edit theme files when needed.
...
it. For occasional cross-repo tasks, Codex can already read files from sibling
...
Currently excludes `AGENTS.md`. If other non-post files are added to the repo root
...
- Created this AGENTS.md with site structure overview, key files, and conventions.
...
- Documented multi-repo website setup and evaluated 4 options for cross-repo
  Codex usage; recommended staying with per-repo sessions (Option A).
...
### 2026-03-01 — AGENTS.md excluded from site rendering
- `AGENTS.md` was showing up as a page in the site nav. Added it to the `exclude`
  list in `_config.yml` so Jekyll ignores it entirely.
- Documented the `exclude` list mechanism in AGENTS.md for future reference.
...
- **Setup guide: Git + Codex workflow for researchers.** Scott Cunningham has
  ...
  The user wants to write a practical setup post covering git + Codex best
  practices, aimed at researchers/academics adopting AI coding tools. Target
  audience is people like Cunningham who are already using Codex but missing
