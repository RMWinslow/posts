...
{
#ifdef SHOW_BUFFER
    float debugWidth = iResolution.y / 3.0;
#else
    ...
    // Set up camera
    // ==========================================================================================
    vec2 cameraAngle = vec2(iTime * TIME_CAM_SPIN + sin(iTime) * TIME_CAM_WOBBLE - PI * 0.9, -0.17 * PI);
    //vec2 cameraAngle = vec2(sin(iTime) * 0.1 - PI * 0.8, -0.17 * PI);
    ...
    else
    {
        ...
        {
#ifndef GREYSCALE
            occlusion = saturate(erosion + 0.5);
            ...
            diffuseColor = mix(diffuseColor, DIRT_COLOR, smoothstep(0.3, 0.0, occlusion - 0.2 + breakup * 1.0));
            ...
            diffuseColor = mix(diffuseColor, grassMix, smoothstep(WATER_HEIGHT + 0.05, WATER_HEIGHT + 0.02, pos.y + 0.01 + (occlusion - 0.8) * 0.05 - breakup * 0.02) * smoothstep(0.8, 1.0, normal.y + breakup * 0.1));
    ...
    debugUV.x = 1.0 - debugUV.x;
    if (debugUV.x > 0.0 && debugUV.x < 1.0 && debugUV.y > 0.0 && debugUV.y < 3.0)
    {
        vec4 tex = GetChannel0(fract(debugUV));
        vec3 normal = tex.yzz;
        normal.y = sqrt(1.0 - dot(normal.xz,normal.xz));
        color = debugUV.y > 2.0 ? normal.xzy * 0.5 + 0.5 : debugUV.y > 1.0 ? tex.xxx * 2.5 - 0.75 : tex.www * 0.5 + 0.5;
    }
#endif
